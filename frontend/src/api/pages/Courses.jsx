  ...
  return (
    <div>
      ...
      {showForm && (
        <form
          onSubmit={create}
          className="card p-5 mb-6 grid gap-4"
        >
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="px-4 py-2 text-gray-400 hover:bg-gray-800 rounded-xl"
            >
              Cancelar
            </button>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-3 py-2.5 bg-gray-950 border border-gray-800 rounded-xl text-white placeholder-gray-600 outline-none focus:ring-2 focus:ring-primary-500"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="card p-5 hover:border-gray-700 transition">
              {editingId === c.id ? (
                <form onSubmit={(e) => saveEdit(e, c.id)} className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button type="button" onClick={cancelEdit} className="px-3 py-2 text-gray-400 hover:bg-gray-800 rounded-lg">Cancelar</button>
                  </div>
                </form>
              ) : (
                <>
                  <Link to={`/courses/${c.id}`} className="block">
                    ...
                    <h3 className="font-semibold text-lg text-white mb-1">{c.name}</h3>
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-400">
                      ...
                      <span className="text-xs bg-gray-800 text-gray-300 px-2 py-1 rounded-full truncate max-w-[140px]">
                        {c.teacher}
                      </span>
                    </div>
                  </Link>
                  {canCreate && (
                    <div className="flex gap-2 mt-4">
                      <button onClick={() => startEdit(c)} className="px-3 py-2 text-sm border border-gray-800 text-gray-300 rounded-lg hover:bg-gray-800">Editar</button>
                      <button onClick={() => deleteCourse(c.id)} className="px-3 py-2 text-sm border border-gray-800 rounded-lg hover:bg-gray-800 text-red-400">Eliminar</button>
                    </div>
                  )}
                </>
              )}
